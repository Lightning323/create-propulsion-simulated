...
public class PlatinumFluidVesselRenderer extends SafeBlockEntityRenderer<PlatinumFluidVesselBlockEntity> {

    public PlatinumFluidVesselRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    protected void renderSafe(PlatinumFluidVesselBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer,
        ...
        float xMax = axis == Direction.Axis.X ? xMin + be.getHeight() - 2 * capSize : xMin + be.getWidth() - 2 * tankHullSize;
        ...
        float zMax = axis == Direction.Axis.Z ? zMin + be.getHeight() - 2 * capSize : zMin + be.getWidth() - 2 * tankHullSize;
    ...
    protected void renderAsBoiler(PlatinumFluidVesselBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer,
                                  int light, int overlay) {
        ...
        Direction.Axis axis = be.getAxis();
        msr.translate(axis == Direction.Axis.X ? be.getHeight() / 2f : be.getWidth() / 2f, 0.5,
            axis == Direction.Axis.Z ? be.getHeight() / 2f : be.getWidth() / 2f);
    ...
    @Override
    public boolean shouldRenderOffScreen(PlatinumFluidVesselBlockEntity be) {
